...
class Tree(Agent):
    ...
    def disperse_seeds(self,fraction=0.5,a=0.3,maximum_distance=100):
        ...
        for d in seed_distances:
            seed_position=self.random_seed_position(distance=d*max_distance)
            self.deposit_seed(seed_position)

    def random_seed_position(self, distance):
        x0,y0=self.position
        angle=np.random.randint(360)
        new_position=((x0+np.cos(np.radians(angle))*distance,y0+np.sin(np.radians(angle))*distance))
    ...
    def DBH_from_Biomass(self,B):
        ...
        #if p>0 and B>0:
        dbh=np.sqrt(B/p)
        if np.isnan(dbh) or dbh<=0: return 0 #if Biomass is negative, new DBH will be 0
        return dbh
