...
class Tree_World(World):
    ...
    def run_simulation(self,n,logging_settings=None,dispersal_settings={'mode':'external','clear':True},produce_fruits=True,increment_time=True,h5file=None,plot_func=None,**kwargs):
        ...
        for i in range(n):
            ...
            if h5file:
               ...
               nee=self.run_schedule(step=self.year,n=n,h5_database=self.db)
            ...
            if dispersal_settings is not None:
                if dispersal_settings['mode']=='external':
                    ...
                    self.incorporate_new_seedbank(ext,clear=dispersal_settings['clear'])
            ...
            if increment_time: self.increment_time()
    ...
    def NEE_data_to_db(self,nee):
           ...
           NEE_table=self.db.get_node("/sim_{0}/trees/sys_lvl/NEE".format(self.sim_number))
           nee_r=NEE_table.row
    ...
    def stocks_data_to_db(self):
        ...
        Stocks_table=self.db.get_node("/sim_{0}/trees/sys_lvl/Stocks".format(self.sim_number))
    ...
    def pop_data_to_db(self):
        ...
        Pop_table=self.db.get_node("/sim_{0}/trees/sys_lvl/Pop".format(self.sim_number))
        pop_r=Pop_table.row
    ...
    def ind_data_to_db(self,step):
        ...
        Ind_table=self.db.get_node("/sim_{0}/trees/ind_lvl/Ind".format(self.sim_number))
        ind_r=Ind_table.row
    ...
    def create_HDF_database(self,database_name):
        ...
        try:
            ...
            sim=h5file.create_group("/","sim_{0}".format(self.sim_number),"Simulation {0}".format(self.sim_number))


        h5file.create_group("/sim_{0}".format(self.sim_number),"trees", "Trees subgroup")
        sys_lvl=h5file.create_group("/sim_{0}/trees".format(self.sim_number),"sys_lvl","System Level Observations for simulation {0}".format(self.sim_number))
    ...
    def log_trees(self, suitable_trees, total_vol):
        ...
        while total_logged<total_vol and len(suitable_trees):
            ...
            print("Logged tree: {0} Volume: {1} AGB: {2}".format(biggest_tree, vols[biggest_tree],Tree.Instances[biggest_tree].AGB))
